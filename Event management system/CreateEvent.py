...
def CreateEvent():
    ...
    def CreateNow():
        if len(event_name.get()) < 5 or len(event_time.get()) == 0 or len(event_duration.get()) == 0:
            show_message('Error', 'Enter valid details for all fields')
            return
    ...
    Button(top2, text='Submit', bg='green', fg='white', font=('Arial', 17), width=9, command=CreateNow).grid(row=6, column=0, pady=10, columnspan=2)
